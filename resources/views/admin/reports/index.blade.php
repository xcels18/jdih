        ...
        <div class="relative z-10 inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-200/80 font-sans">
            ...
            <div class="bg-slate-50 px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <a id="modal-wa-reply" href="#" target="_blank" class="hidden bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-extrabold uppercase tracking-wider py-2.5 px-5 rounded-full shadow-md shadow-emerald-600/10 transition hover:scale-[1.02] flex items-center gap-1.5">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function openModal(data) {
            ...
            if (cleanNumber.length >= 9 && (contactText.startsWith('0') || contactText.startsWith('62') || contactText.startsWith('+62') || contactText.startsWith('+'))) {
                ...
                // Set up WhatsApp reply pre-filled message (Pelayanan Prima)
                const nameParam = data.name !== 'Masyarakat Umum' ? data.name : 'Bapak/Ibu';
                const truncatedMsg = data.message.length > 80 ? data.message.substring(0, 80) + '...' : data.message;
                
                const waTemplate = `Yth. ${nameParam},

Terima kasih telah menghubungi JDIH Kabupaten Puncak Jaya. Kami sangat mengapresiasi laporan/saran Bapak/Ibu mengenai:

"${truncatedMsg}"

Laporan tersebut saat ini sedang ditindaklanjuti oleh tim admin JDIH. Kami berkomitmen untuk terus memberikan pelayanan prima dalam penyebarluasan dokumentasi & informasi hukum.
...
Tim Pengelola JDIH Puncak Jaya`;
                ...
                modalWaReply.classList.add('hidden');
            }
